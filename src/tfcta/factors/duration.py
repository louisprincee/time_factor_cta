...
def rolling_threshold_grid(abs_diff: np.ndarray,
    ...
    为什么要有这个函数
    ------------------
    第 3 步要跑 3 个 N × 5 个 M = 15 个组合。逐个调用会把「按日切分样本」做 15 遍、
    把「拼接滚动窗口」做 15 遍，而这两件事里只有最后取分位数那一步与 M 有关。
    这里改成：切分做 1 遍，每个 N 拼接 1 遍，然后 ``np.percentile`` 一次给出 5 个
    分位数——工作量从 15 遍降到 3 遍，逐元素结果完全一致（有测试钉住）。
